...
# ----- Download model from Google Drive with retry -----
file_id = "17YoKDtwrGL8YzRv9J3IRRnAHpAzaVYb7"   # replace with your file ID
...
for i in range(3):  # try 3 times
    try:
        ...
        st.warning(f"Download failed, retrying {i+1}/3...")
        time.sleep(5)
        if i == 2:
            st.error("Failed to download model after 3 attempts. Please check the file or your internet connection.")
            st.stop()
...
team_dict = {
    ...
    "PSG": 4,
    "Bayern Munich": 5,
    # add all teams you used in training
}
...
# ----- Reverse dicts for displaying -----
inv_team_dict = {v: k for k, v in team_dict.items()}
inv_position_dict = {v: k for k, v in position_dict.items()}
...
# ----- Single Player Prediction Page -----
    ...
    position = st.selectbox("Select Position:", list(position_dict.keys()))
    ...
    if st.button("Predict Transfer Fee"):
        team_encoded = team_dict[team]
        position_encoded = position_dict[position]
...
# ----- Compare Two Players Page -----
    ...
    team1 = st.selectbox("Team P1:", list(team_dict.keys()), key="t1")
    position1 = st.selectbox("Position P1:", list(position_dict.keys()), key="p1")
    ...
    team2 = st.selectbox("Team P2:", list(team_dict.keys()), key="t2")
    position2 = st.selectbox("Position P2:", list(position_dict.keys()), key="p2")
    ...
    if st.button("Compare Transfer Fees"):
        team1_encoded = team_dict[team1]
        position1_encoded = position_dict[position1]
        team2_encoded = team_dict[team2]
        position2_encoded = position_dict[position2]
